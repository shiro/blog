...
import {
  ...
  ThemedToken,
  ThemeRegistrationResolved,
  TokenizeWithThemeOptions as BaseTokenizeWithThemeOptions,
} from "@shikijs/core";

interface TokenizeWithThemeOptions extends BaseTokenizeWithThemeOptions {
  cursorInfo?: {
    x: number;
    y: number;
    shape: "underline" | "block" | "bar" | "default";
  };
}
...
export function tokenizeAnsiWithTheme(
  ...
  const cursorInfo = options?.cursorInfo;

  const lines = splitLines(fileContents, true);
  ...
  return lines.map(([line, lineOffset]) =>
    parser.parse(line).map((token): ThemedToken => {
      ...
      return {
        ...
        bgColor,
        fontStyle,
        htmlAttrs: { class: token.decorations.has("reverse") ? "reverse" : "" },
      };
    })
  );
}
