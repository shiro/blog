...
export async function resolve(specifier, context, nextResolve) {
  try {
    ...
    if (
      ...
      resolved = await tryResolve(`${specifier}.ts`);
      if (resolved) return resolved;

      resolved = await tryResolve(`${specifier}.tsx`);
      if (resolved) return resolved;
  ...
  if (urlUrl.protocol === "file:") {
    ...
    if (SUPPORTED_EXTENSIONS.includes(extension)) {
      return defaultGetFormat(
        replaceExtension(urlUrl, extension, ".js"),
        context,
        defaultGetFormat,
      );
    }
...
export async function load(url, context, nextLoad) {
  const { format, source } = await nextLoad(url, context).catch(
